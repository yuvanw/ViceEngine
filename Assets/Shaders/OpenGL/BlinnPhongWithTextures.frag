...
{
	vec3 DiffuseColor = vec3(texture(Material.Diffuse, Input.TextureCoordinates));

	// Ambient
    vec3 Ambient = InLight.Color * DiffuseColor;
  	
    // Diffuse 
	vec3 LightDirection = normalize(-InLight.Direction);
    float DiffuseStrength = max(dot(InNormal, LightDirection), 0.0);
    vec3 Diffuse = InLight.Color * DiffuseStrength * DiffuseColor;
    ...
    vec3 Specular = InLight.Color * vec3(texture(Material.Specular, Input.TextureCoordinates)) * SpecularStrength;
